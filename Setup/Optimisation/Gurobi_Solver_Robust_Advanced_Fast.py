...
class Gurobi_Solver_Robust_Advanced(Gurobi_Solver):
    ...
    def __init__(self, number_of_satellites: int, prediction_horizon: int, model: LTV_System,
        ...
        self.x_lim[0] = np.inf
        self.u_lim = np.tile(input_limit, (self.prediction_horizon, self.number_of_satellites))
        ...
        self.phi_x_pos = self._problem.addMVar(number_of_blocks * self.x_vars, name='phi_x_pos')  # , lb=0, ub=np.inf)
        self.phi_x_neg = self._problem.addMVar(number_of_blocks * self.x_vars, name='phi_x_neg')  # , lb=0, ub=np.inf)
        self.phi_x_abs = self._problem.addMVar(number_of_blocks * self.x_vars, name='phi_x_abs')  # , lb=0, ub=np.inf)

        self.phi_u_pos = self._problem.addMVar(number_of_blocks * self.u_vars, name='phi_u_pos')  # , lb=0, ub=np.inf)
        self.phi_u_neg = self._problem.addMVar(number_of_blocks * self.u_vars, name='phi_u_neg')  # , lb=0, ub=np.inf)
        self.phi_u_abs = self._problem.addMVar(number_of_blocks * self.u_vars, name='phi_u_abs')  # , lb=0, ub=np.inf)

        self.x_pos = self._problem.addMVar(prediction_horizon * self._nx, name='x_pos')  # , lb=0, ub=x_max)
        self.x_neg = self._problem.addMVar(prediction_horizon * self._nx, name='x_neg')  # , lb=0, ub=x_max)
        self.x_abs = self._problem.addMVar(prediction_horizon * self._nx, name='x_abs')  # , lb=0, ub=x_max)

        self.u_pos = self._problem.addMVar(prediction_horizon * self._nu, name='u_pos')  # , lb=0, ub=u_max)
        self.u_neg = self._problem.addMVar(prediction_horizon * self._nu, name='u_neg')  # , lb=0, ub=u_max)
        self.u_abs = self._problem.addMVar(prediction_horizon * self._nu, name='u_abs')  # , lb=0, ub=u_max)

        self.x_max = self._problem.addMVar(prediction_horizon, name='x_max')
        ...
        self._problem.addConstr(self.phi_x_pos - self.phi_x_neg == self.phi_x)
        self._problem.addConstr(self.phi_x_pos + self.phi_x_neg == self.phi_x_abs)

        self._problem.addConstr(self.phi_u_pos - self.phi_u_neg == self.phi_u)
        self._problem.addConstr(self.phi_u_pos + self.phi_u_neg == self.phi_u_abs)

        self._problem.addConstr(self.x_pos - self.x_neg == self.x)
        self._problem.addConstr(self.x_pos + self.x_neg == self.x_abs)

        self._problem.addConstr(self.u_pos - self.u_neg == self.u)
        self._problem.addConstr(self.u_pos + self.u_neg == self.u_abs)
        ...
        for n in range(number_of_blocks):
            ...
            self._problem.addConstr(one_norm_matrix_u @ self.phi_u_abs[n * self.u_vars:(n + 1) * self.u_vars] <= self.phi_u_max[n])
    ...
    def update_x0(self, time_since_start: int = 0) -> None:
        ...
        self.constraint_list.append(self._problem.addConstr(
            self.epsilon_matrix_A * np.max(np.abs(x0)) + self.epsilon_matrix_B * self.u_max[0] + self.sigma_w <= self.sigma[:self._nx]))
