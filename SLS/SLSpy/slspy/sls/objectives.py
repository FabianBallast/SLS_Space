...
class SLS_Obj_LQ(SLS_Objective):
    ...
    def addObjectiveValue(self, sls, objective_value, x_ref=None):
        Q_sqrt = self._Q_sqrt
        R_sqrt = self._R_sqrt
        ...
        if sls._noise_free:
            self.state_est = sls._system_model._x0
            # self.state_est = cp.Parameter(sls._system_model._x0.shape)  # x0 is the known state when noise free
        else:
            if Cov_w_sqrt is None:
                Cov_w_sqrt = sls._system_model._B1
            self.state_est = Cov_w_sqrt  # State unknown, but cov_w gives a guess

        if Cov_v_sqrt is None:
            Cov_v_sqrt = sls._system_model._D21
        if x_ref is None:
            x_ref = np.zeros_like(sls._system_model._x0)
        ...
        if sls._state_feedback:
            ...
            for tau in range(len(Phi_x)):
                self._objective_expression += cp.sum_squares(Q_sqrt @ (Phi_x[tau] @ self.state_est - x_ref))
                self._objective_expression += cp.sum_squares(R_sqrt @ Phi_u[tau] @ self.state_est)
        else:
            # output-feedback
            Phi_xx = sls._Phi_xx
            Phi_ux = sls._Phi_ux
