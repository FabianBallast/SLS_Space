...
class LTI_System(SystemModel):
    ...
    def initialize(self, x0=None):
        ...
        if x0 is None:
            ...
            if self._x0.shape[0] != self._Nx:
                return self.errorMessage(f"Initial state should be of shape ({self._Nx}, 1),"
                                         f"but is of shape {self._x0.shape}.")
...
class LTV_System(SystemModel):
    """
    Contains all matrices of an LTV system
    """

    def __init__(self, Nx: int = 0, Nw: int = 0, Nu: int = 0, Ny: int = 0, Nz: int = 0, tFIR: int = 0, **kwargs):
        SystemModel.__init__(self, **kwargs)
        ...
        # state       : x(t+1)= A_t*x_t  + B1_t*w_t  + B2_t*u_t
        self._A = None  # np.zeros([tFIR, Nx,Nx])
        self._B1 = None  # np.zeros([tFIR, Nx,Nw])
        self._B2 = None  # np.zeros([tFIR, Nx,Nu])
        ...
        self._Nz = Nz  # output
        self._Ny = Ny  # measurement
        self._tFIR = tFIR  # FIR length of LTV system, e.g. number of A matrices considered
    def initialize(self, x0=None):
        SystemModel.initialize(self)
        ...
        if not self._ignore_output:
            if self._C1 is None:
                self.errorMessage(
                    'C1 is not defined when the system output (z) is not ignored. Initialization fails.')
            else:
                self._z = np.dot(self._C1, self._x)
    ...
    def sanityCheck(self):
        ...
        Nx = self._Nx
        Nw = self._Nw
        Nu = self._Nu = self._B2.shape[2]
        tFIR = self._tFIR

        # fill in zero matrices if undefined
        if self._A is None:
            self._A = np.zeros([tFIR, Nx, Nx])
        elif ((self._A.shape[0] != tFIR) or
              (self._A.shape[1] != Nx) or
              (self._A.shape[2] != Nx)):
            return self.errorMessage('Dimension mismatch: A')

        if self._B1 is None:
            self._B1 = np.zeros([tFIR, Nx, Nw])
        elif ((self._B1.shape[0] != tFIR) or
              (self._B1.shape[1] != Nx) or
              (self._B1.shape[2] != Nw)):
            return self.errorMessage('Dimension mismatch: B1')

        if self._B2 is None:
            self._B2 = np.zeros([tFIR, Nx, Nu])
        elif ((self._B2.shape[0] != tFIR) or
              (self._B2.shape[1] != Nx) or
              (self._B2.shape[2] != Nu)):
            return self.errorMessage('Dimension mismatch: B2')
